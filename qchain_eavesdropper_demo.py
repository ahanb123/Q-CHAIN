...
def qubits_to_int(qubits):
    ...
    Includes error handling for qubits destroyed by an eavesdropper.
    """
    binary = ""
    for q in qubits:
        if q is not None:
            try:
                # Attempt to measure the received qubit
                binary += str(q.measure())
            except Exception:
                # If Eve intercepted it, the backend throws a KeyError.
                # We catch it and log it as corrupted data.
                binary += "0" 
        else:
            binary += "0"
...
def alice_protocol_signature(host, transaction, results):
    ...
    Steps:
        ...
        4. Transmit signature via direct quantum transmission
    ...
    # Direct quantum transmission of signature
    for idx, sig_component in enumerate(signature):
        print(f"[Alice] Transmitting component {idx+1}/{len(signature)}...")
        
        sig_qubits = encode_int_to_qubits(host, sig_component, num_qubits)
        
        for q_sig in sig_qubits:
            host.send_qubit('Bob', q_sig, await_ack=True)
...
def bob_protocol_signature(host, results):
    ...
    Steps:
        ...
        3. Receive signature via direct quantum transmission
        4. Verify signature
    ...
    # Receive signature via direct quantum transmission
...
def eve_sniffing_quantum(sender, receiver, qubit):
    """
    Eve intercepts the quantum transmission.
    By measuring the qubit traversing the network, Eve destroys the entanglement 
    (no-cloning theorem), simulating a tamper-evident physical layer attack.
    """
    global eve_interceptions
    if eve_interceptions == 0:
        print("\n[Eve] EAVESDROPPING DETECTED: Intercepting and measuring passing qubits...")
    eve_interceptions += 1
    
    # Measurement forces wave-function collapse
    qubit.measure()
...
def main():
    ...
    This demonstrates:
        ...
        - Tamper-evidence via an active eavesdropper (Eve)
    """
    results = {
        'keys': None,
        'signature': None,
    ...
    try:
        ...
        # Initialize hosts including Eve
        alice = Host('Alice')
        bob = Host('Bob')
        eve = Host('Eve')
        
        # Configure topology to force routing through Eve: Alice <-> Eve <-> Bob
        alice.add_connection('Eve')
        eve.add_connection('Alice')
        eve.add_connection('Bob')
        bob.add_connection('Eve')
        
        network.add_hosts([alice, bob, eve])
        
        # Enable quantum eavesdropping for Eve
        eve.q_relay_sniffing = True
        eve.q_relay_sniffing_fn = eve_sniffing_quantum
        ...
        print(f"Transmission: DIRECT QUANTUM CHANNEL (Routed via Eve)")
        
        print(f"\n[Security Event] Eve intercepted and measured {eve_interceptions} qubits.")
        ...
        if verification_result:
            ...
            print("TAMPER EVIDENCE DEMONSTRATED: Eavesdropping collapsed the quantum state,")
            print("destroying the signature before Bob could authenticate the transaction.")
            print("="*80)
